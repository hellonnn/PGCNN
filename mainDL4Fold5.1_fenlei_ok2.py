...
# CLASS_WEIGHTS = [1.0, 15.0, 15.0, 20.0]
...
if __name__ == "__main__":
    ...
    #     # 构建并训练模型
    # model = build_pgnn_classification_model(
    #     input_shape=(patch_row_col_Num, patch_row_col_Num, inpFea_num),
    #     rain_period=rain_period
    # )
    # model.summary()
    ...
    # # 保存模型
    # model.save('result3/高斯消融/junction/modelSaver_pgnn_classification_ok.h5')
    ...
    # plt.tight_layout()
    # plt.savefig('result3/高斯消融/junction/pgnn_classification_performance_5.ok4_XR.png')
    # plt.show()
    
    # # 用于累积统计
    # total_pixels = np.zeros(NUM_CLASSES, dtype=np.int64)
    # correct_pixels = np.zeros(NUM_CLASSES, dtype=np.int64)

    # # 遍历验证集
    # for (cov_patch, rain_vec), y_true in val_ds:
    #     # model.predict 支持 Dataset
    #     y_pred = model.predict((cov_patch, rain_vec), verbose=0)
    #     y_pred = np.argmax(y_pred, axis=-1)  # (B, 64, 64)

    #     y_true = y_true.numpy()  # (B, 64, 64, 1)
    #     y_true = y_true[..., 0]  # (B, 64, 64)

    #     for cls in range(NUM_CLASSES):
    #         mask = (y_true == cls)
    #         total_pixels[cls] += np.sum(mask)
    #         correct_pixels[cls] += np.sum((y_pred == cls) & mask)

    # # 计算各类别准确率
    # class_acc = {}
    # for cls in range(NUM_CLASSES):
    #     if total_pixels[cls] > 0:
    #         class_acc[cls] = correct_pixels[cls] / total_pixels[cls]
    #     else:
    #         class_acc[cls] = 0.0

    # # 打印 & 保存
    # print("\n各类别验证准确率:")
    # with open('result3/高斯消融/junction/classification_info_5.ok4_XR.txt', 'w') as f:
    #     f.write(f"分类阈值: {DEPTH_THRESHOLDS}\n")
    #     f.write("各类别验证准确率:\n")
    #     for cls, acc in class_acc.items():
    #         print(f"类别 {cls}: {acc:.4f}")
    #         f.write(f"类别 {cls}: {acc:.4f}\n")