...
# USE_SLOPE      = True   # 坡度
# USE_ASPECT     = True  # 坡向
# USE_CURVATURE  = True   # 曲率
# USE_BUILDING   = True   # 建筑物
# USE_PIPE       = True  # 管径
# USE_JUNCTION   = True   # 检查井

# def physical_constraints_module(dem, slope, aspect, curvature,
#                                 building, pipe, junction):
#     """
#     带“高斯消融”开关的物理约束模块
#     若开关为 False，则用与当前批次同分布的高斯噪声替代原张量
#     """
#     import tensorflow as tf
#     from tensorflow.keras.layers import (Conv2D, BatchNormalization,
#                                          Concatenate, Multiply, Subtract, ReLU)

#     def _gaussian_ablate(tensor, flag):
#         """
#         flag=True  : 返回原张量
#         flag=False : 返回同 shape 的高斯噪声，均值为 0，标准差与当前 tensor 相同
#         """
#         if flag:
#             return tensor
#         # 仅在训练阶段用随机噪声，推断阶段也可用固定噪声或 0
#         noise = tf.random.normal(tf.shape(tensor),
#                                  mean=0.0,
#                                  stddev=tf.nn.moments(tensor, axes=None)[1])  # 标准差
#         return noise

#     # 1. 高斯消融
#     dem        = _gaussian_ablate(dem,        USE_DEM)
#     slope      = _gaussian_ablate(slope,      USE_SLOPE)
#     aspect     = _gaussian_ablate(aspect,     USE_ASPECT)
#     curvature  = _gaussian_ablate(curvature,  USE_CURVATURE)
#     building   = _gaussian_ablate(building,   USE_BUILDING)
#     pipe       = _gaussian_ablate(pipe,       USE_PIPE)
#     junction   = _gaussian_ablate(junction,   USE_JUNCTION)
...
#     aspect_feat  = Conv2D(8, (3,3), padding='same', activation='relu')(aspect)
#     curv_feat    = Conv2D(8, (3,3), padding='same', activation='relu')(curvature)
#     terrain_inflow = Concatenate(axis=-1)([flow_accumulation, aspect_feat, curv_feat])
#     terrain_inflow = Conv2D(16, (1,1), padding='same', activation='relu')(terrain_inflow)
...
#     # ---- 5. 检查井净风险 ----
#     junction_inflow = Multiply()([flow_accumulation, junction])
#     net_risk = Subtract()([junction_inflow, pipe_capacity])
#     net_risk = ReLU()(net_risk)
#     junction_effect = net_risk
...
# #-------------------温和消融实验-------------------------------------------------
# def physical_constraints_module(dem, slope, aspect, curvature, building, pipe, junction,
#                                 ablate_flow=False):   # <-- 新增开关
#     """物理约束处理模块（支持消融 flow_accumulation）"""
#     # 标准化输入
#     dem        = BatchNormalization()(dem)
#     slope      = BatchNormalization()(slope)
#     aspect     = BatchNormalization()(aspect)
...
#     # # --------------  消融处  --------------
#     # if ablate_flow:
#     #     flow_accumulation = Lambda(lambda x: x * 0., name='ablate_flow')(flow_accumulation)
#     # # --------------------------------------
...
#     # # --------------  消融处  --------------
#     # if ablate_flow:
#     #     terrain_building = Lambda(lambda x: x * 0., name='ablate_flow')(terrain_building)
#     # # --------------------------------------
...
#     # # --------------  消融处  --------------
#     # if ablate_flow:
#     #     pipe_capacity = Lambda(lambda x: x * 0., name='ablate_flow')(pipe_capacity)
#     # # --------------------------------------
...
#     # --------------  消融处  --------------
#     if ablate_flow:
#         junction_effect = Lambda(lambda x: x * 0., name='ablate_flow')(junction_effect)
#     # --------------------------------------

#     # 6) 合并
#     physical_features = Concatenate(axis=-1)([
#         flow_accumulation, terrain_inflow, terrain_building,
#         pipe_capacity, junction_effect
#     ])
#     physical_features = Conv2D(64, (3,3), padding='same', activation='relu')(physical_features)
#     return BatchNormalization()(physical_features)
...
#     """
#     带消融开关的物理约束模块
#     若某个开关为 False，则对应张量用全 0 同 shape 张量替代
#     """
#     import tensorflow as tf
#     def _mask(t, flag):
#         return t if flag else tf.zeros_like(t)

#     dem        = _mask(dem,        USE_DEM)
#     slope      = _mask(slope,      USE_SLOPE)
#     aspect     = _mask(aspect,     USE_ASPECT)
#     curvature  = _mask(curvature,  USE_CURVATURE)
#     building   = _mask(building,   USE_BUILDING)
#     pipe       = _mask(pipe,       USE_PIPE)
#     junction   = _mask(junction,   USE_JUNCTION)

#     # 以下与原版完全一致（所有张量已被 0 填充）
#     dem        = BatchNormalization()(dem)
#     slope      = BatchNormalization()(slope)
#     aspect     = BatchNormalization()(aspect)
#     curvature  = BatchNormalization()(curvature)
...
#     # ---- 1. 汇流累积量（依旧依赖 DEM，如果 DEM 被置 0 则汇流为 0） ----
#     flow_accumulation = Conv2D(16, (3,3), padding='same', activation='relu')(dem)
#     flow_accumulation = Conv2D(16, (3,3), padding='same', activation='relu')(flow_accumulation)
...
# #---------------------高斯噪声消融--------------------------------------
# import tensorflow as tf
# from tensorflow.keras.layers import BatchNormalization, Conv2D, Concatenate, Multiply, Subtract, ReLU, Lambda

# def gaussian_ablate(x, scale=0.5):
#     """给特征加高斯噪声，scale 越大破坏越强"""
#     noise = tf.random.normal(shape=tf.shape(x), mean=0.0, stddev=scale, dtype=x.dtype)
#     return x + noise

# def physical_constraints_module(
#     dem, slope, aspect, curvature, building, pipe, junction,
#     ablate_flow=False,           # 全局总开关（向后兼容）
#     ablate_flow_accum=False,     # 1) 汇流累积量
#     ablate_terrain_in=False,     # 2) 地形入流
#     ablate_building=False,       # 3) 建筑
#     ablate_pipe=False,           # 4) 管网
#     ablate_junction=True,       # 5) 节点
#     noise_scale=0.5
# ):
#     """物理约束特征提取（支持逐分支高斯噪声消融）"""
...
#     flow_accumulation = Conv2D(16, (3, 3), padding='same', activation='relu')(dem)
#     flow_accumulation = Conv2D(16, (3, 3), padding='same', activation='relu')(flow_accumulation)
#     if ablate_flow or ablate_flow_accum:
#         flow_accumulation = Lambda(
#             lambda x: gaussian_ablate(x, noise_scale)
#         )(flow_accumulation)
...
#     terrain_inflow = Concatenate(axis=-1)([flow_accumulation, aspect_feat, curv_feat])
#     terrain_inflow = Conv2D(16, (1, 1), padding='same', activation='relu')(terrain_inflow)
#     if ablate_flow or ablate_terrain_in:
#         terrain_inflow = Lambda(
#             lambda x: gaussian_ablate(x, noise_scale)
#         )(terrain_inflow)
...
#     terrain_building = Multiply()([dem, building])
#     terrain_building = Conv2D(16, (3, 3), padding='same', activation='relu')(terrain_building)
#     if ablate_flow or ablate_building:
#         terrain_building = Lambda(
#             lambda x: gaussian_ablate(x, noise_scale)
#         )(terrain_building)
...
#     pipe_capacity = Multiply()([pipe, slope])
#     pipe_capacity = Conv2D(16, (3, 3), padding='same', activation='relu')(pipe_capacity)
#     if ablate_flow or ablate_pipe:
#         pipe_capacity = Lambda(
#             lambda x: gaussian_ablate(x, noise_scale)
#         )(pipe_capacity)
...
#     net_risk = ReLU()(net_risk)
#     junction_effect = net_risk
#     if ablate_flow or ablate_junction:
#         junction_effect = Lambda(
#             lambda x: gaussian_ablate(x, noise_scale)
#         )(junction_effect)
