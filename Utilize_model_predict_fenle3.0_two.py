...
valid_path = "rain_pr/Rain50Type5.asc"  # validSet数据

# 定义研究区的行列数
rowNum = 656
colNum = 650
scnNum = 6
RAIN_SCENARIO_INDEX = 4  # 手动设置需要预测的降雨场景索引（0~5）
rain_period = 12
...
def classify_depth(depth_data, impossible_mask):
    classified = np.zeros_like(depth_data, dtype=np.uint8)  # 默认 0 类 = 无水
    valid = ~impossible_mask
    
    # 只要大于0，就归为“有水”= 1 类
    mask1 = valid & (depth_data > 0)
    classified[mask1] = 1
    return classified
...
CLASS_WEIGHTS = tf.constant([1.0, 5.0], dtype=tf.float32)  # 比如无水权重1，有水权重5
...
model_path = 'result4/测试/modelSaver_pgnn_classification_ok.h5'  # 注意使用4分类模型
...
# 保存预测结果（分类索引）
np.savetxt(r'result4/测试/DHC_class_predict_1.txt', pred_class_whole, fmt='%d', delimiter=' ')                       #----------------------------------------------------------
...
def plot_classification_result(prediction, ground_truth, prefix):
    ...
    ax.set_title('Model Prediction (TEST1)', fontsize=14)            #----------------------------------------------------------
    ...
    try:
        ...
        cax = divider.append_axes('left', size='5%', pad=0.5)  # cm
    ...
    ax.set_title('Ground Truth (TEST1)', fontsize=14)            #----------------------------------------------------------
    ...
    try:
        ...
        divider = make_axes_locatable(ax)
        cax = divider.append_axes('right', size='5%', pad=0.5)  # cm
        cbar = fig.colorbar(im, cax=cax)
...
def plot_diff_map_new(prediction, ground_truth, filename):
    ...
    # 4. 颜色映射
    diff_colors = ["#ffffff", "#e21e1e"]   # 白=一致, 红=错误
    ...
    plt.title("Error Category Map (TEST1)", fontsize=14)            #----------------------------------------------------------
...
# # 可视化分类结果
# plot_classification_result(pred_class_whole, valid_class, "result4/测试/classification_comparison_ok1")          #----------------------------------------------------------


# 新增差值图
plot_diff_map_new(pred_class_whole,
                  valid_class,
                  "result4/测试/error_map_ok1.png")                 #----------------------------------------------------------
# 计算并打印准确率
...
# 保存准确率结果
with open('result4/测试/accuracy_report_ok1.txt', 'w') as f:         #----------------------------------------------------------
    plt.tight_layout()
    f.write("4分类模型性能报告\n")  # 更新报告标题
    f.write("="*50 + "\n")
    ...
    f.write("二分类模型性能报告\n")
    f.write("类别定义:\n")
    f.write("  0: 无水 (depth = 0)\n")
    f.write("  1: 有水 (depth > 0)\n\n")
    ...
    plt.title("Confusion Matrix (TEST1)")                    #----------------------------------------------------------
    plt.tight_layout()      
    plt.savefig('result4/测试/confusion_matrix_pct1.png', dpi=300, bbox_inches='tight')      #----------------------------------------------------------
    plt.tight_layout()
    plt.close()
